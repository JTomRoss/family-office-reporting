"""
Parser: Goldman Sachs – Cuenta Custodia (Cartola PDF).

AISLADO: No comparte lógica con JPMorgan.
"""
...
class GoldmanSachsCustodyParser(BaseParser):
    ...
    VERSION = "1.0.0"
    DESCRIPTION = "Parser para cartolas Goldman Sachs custodia (PDF)"
    ...
    def validate(self, result: ParseResult) -> list[str]:
        ...
        if result.opening_balance is not None and result.closing_balance is not None:
            ...
            if diff > Decimal("0.01"):
                errors.append(
                    f"GS balance check: expected {expected}, got {result.closing_balance}"
                )
        return errors
